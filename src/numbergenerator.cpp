 ...
 */
float NumberGenerator::noise(float x) {
...
float NumberGenerator::randomFloat(float min, float max) {
    ...
    std::uniform_real_distribution<float> randomFloatGenerator{min, max};

    return randomFloatGenerator(engine);
}

int NumberGenerator::randomInt(int min, int max) {
    std::random_device dev{};
    std::mt19937 engine{dev()};
    std::uniform_int_distribution<int> randomIntGenerator{min, max};

    return randomIntGenerator(engine);
}
